...
int8_t Play(const ChessGameState *cgs, Move *bestMove, ChessGameState *nextCgs)
{
...
ChessGameState NondetSituation()
{
  // TODO: implement
}
...
int main() {
  // The starting position (default game start or another situation)
  ChessGameState startPos;
  InitSituation(&startPos);

  ChessGameState endPos = NondetSituation();

  Move firstMove;
  ChessGameState nextPos, prevPos;
  // If it's opponent's turn and opponent evaluates the game as lost
  int8_t outcome = Play(&endPos, &firstMove, &nextPos);
  if ( (startPos.blacksTurn_ != endPos.blacksTurn_ && outcome < 0) || (startPos.blacksTurn_ == endPos.blacksTurn_ && outcome > 0) )
  {
    ChessGameState curPos = endPos;
    for (int i=0; i<MAX_SEARCH_DEPTH; i++)
    {
      prevPos = NondetSituation();
      outcome = Play(&prevPos, &firstMove, &nextPos);
      if (!IsSameCGS(nextPos, curPos))
      {
        break;
      }
      __CPROVER_assert(!IsSameCGS(prevPos, startPos), "Starting player forces a win");
      curPos = prevPos;
    }
  }
  return 0;
