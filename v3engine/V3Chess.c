...
CheckState GetCheckState(const ChessGameState *cgs, bool whiteKing) {
  const int8_t kingRow = (whiteKing ? cgs->whiteKingRow_ : cgs->blackKingRow_);
  const int8_t kingCol = (whiteKing ? cgs->whiteKingCol_ : cgs->blackKingCol_);
  ...
  {
    const int8_t oppPawnRow = (whiteKing ? cgs->whiteKingRow_+1 : cgs->blackKingRow_-1);
    const int8_t oppPawnCol = (whiteKing ? cgs->whiteKingCol_+dc : cgs->blackKingCol_+dc);
  ...
  for (int8_t iDir=0; iDir<4; iDir++)
  {
    ...
    {
      const int8_t oppRookRow = (whiteKing ? cgs->whiteKingRow_ : cgs->blackKingRow_) + k * cRookDirs[iDir][0];
      const int8_t oppRookCol = (whiteKing ? cgs->whiteKingCol_ : cgs->blackKingCol_) + k * cRookDirs[iDir][1];
  ...
  for (int8_t iDir=0; iDir<4; iDir++)
  {
    for (int8_t k=1; k<=7; k++)
    {
      const int8_t oppRookRow = (whiteKing ? cgs->whiteKingRow_ : cgs->blackKingRow_) + k * cBishopDirs[iDir][0];
      const int8_t oppRookCol = (whiteKing ? cgs->whiteKingCol_ : cgs->blackKingCol_) + k * cBishopDirs[iDir][1];
      if (!IsOnBoad(oppRookRow, oppRookCol))
      {
        break;
      }
